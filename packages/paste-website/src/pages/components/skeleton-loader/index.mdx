...
Use the `height` prop to change the height of the Skeleton Loader. The height can be any valid height unit, such as `px`, `rem`, or `%`. It can also be one of the [Paste size tokens](/tokens/#sizings).
